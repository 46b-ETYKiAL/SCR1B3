...
fn assert_arm(app: &ScribeApp, out: &egui::FullOutput, arm: Arm) {
    ...
    match arm {
        Arm::SRope | Arm::GRope => assert!(
            app.tabs[app.active].text.rope_state().is_some(),
...
fn two_pane_grid(arm: Arm) -> ScribeApp {
    ...
    app.tabs[1].set_text(text.clone());
    if arm == Arm::GRo {
...
fn promotable_grid() -> ScribeApp {
    ...
    cfg.spellcheck.enabled = false;
    let mut app = ScribeApp::new_test(cfg);
    app.tabs[0].set_text(under.clone());
    app.tabs.push(EditorTab::scratch());
    app.tabs[1].set_text(under);
    app.active = 0;
    app
}
...
fn promotable_in(vault: Option<&std::path::Path>) -> Promotable {
    ...
    assert!(
        app.tabs[0].text.rope_state().is_none(),
...
fn promote(f: &mut Promotable) {
    f.app.tabs[0].set_text(over_threshold_list());
    f.p.idle(&mut f.app);
    ...
    assert!(
        f.app.tabs[0].text.rope_state().is_some(),
