...
fn render_sections(
    ...
    // ---- Appearance ----
    changed |= appearance::render(ui, config, &def, sel, q);

    // ---- Fonts ----  (no ligatures: egui has no OpenType shaping, so the
    // toggle is intentionally absent rather than shown as a dead control.)
    changed |= fonts::render(ui, config, &def, sel, q);

    // ---- Editor ----
    changed |= editor::render(ui, config, &def, sel, q);

    // ---- Motion / animations ----
    changed |= motion::render(ui, config, &def, sel, q);

    // ---- Window (transparency / glass) ----
    changed |= window::render(ui, config, &def, sel, q);
    ...
    // ---- Spellcheck ----
    changed |= spellcheck::render(ui, config, &def, sel, q);

    // ---- Updates ----
    changed |= updates::render(ui, config, &def, updater, sel, q);

    // ---- Privacy ----
    changed |= privacy::render(ui, config, sel, q);

    // ---- Plugins ----
    changed |= plugins::render(ui, config, &def, sel, q);
    ...
    changed |= default_app::render(ui, config, sel, q);
